...
int main() {
    ...
    Staffer Wade = Staffer("LAB", 20);
    Staffer Mattox = Staffer("MP", 10);

    vector<Staffer> onDutyStaff {Wade, Mattox};

    int expectedWaitTime = waitTime(officeHourQueue, onDutyStaff, 5);

    cout << "The expected wait time for TimTim, the fourth student in the queue, is " << to_string(expectedWaitTime) << " minutes." << endl;
